...
class AddMeal extends Component{
    ...
    render() {
        ...
        <ul className="navbar-nav bg-secondary-color sidebar sidebar-dark accordion" 
            id="accordionSidebar">
        {/* Sidebar - Brand */}
        ...
        <div className='card mx-auto rounded-top-4 rounded-bottom-4 shadow' 
            style={{minWidth: "480px", maxWidth: "600px"}}>
        <img className="card-img-top rounded-top-4 bottom-top-4" src={mealBanner} 
            alt="Add meal banner" 
            style={{height: "200px",  objectFit: "cover", objectPosition: "center"}}/>
           
        <form onSubmit={this.addMeal} className="validated px-4 satoshi"> {/****** FORM */}
            ...
            <div className="mb-3">
                <input type="text" className="form-control" placeholder="Meal name" required
                    value={this.state.meal_name} onChange={this.mealNameChange} />
            </div>
            <div className="mb-3">
                <textarea className="form-control" placeholder="Meal details" rows="3" required
                    value={this.state.meal_desc} onChange={this.mealDescChange} />
            </div>
            <div className="mb-3">
                <textarea className="form-control" placeholder="Meal nutrients" rows="2" required
                    value={this.state.meal_nutri} onChange={this.mealNutriChange} />
            </div>
            <div className='d-flex d-grid justify-content-end gap-3 mb-3'>
                <div className="dropdown">
                <button className="btn btn-secondary bg-secondary-color border-0 dropdown-toggle" 
                        type="button" id="dropdownCategory" data-bs-toggle="dropdown" 
                ...
                <ul className="dropdown-menu" aria-labelledby="dropdownCategory">
                    <li><button className="dropdown-item" type='button' 
                        value="Plant" onClick={this.categoryChange}>Plant Based</button></li>
                    <li><button className="dropdown-item" type='button' 
                        value='Meat' onClick={this.categoryChange}>Meat Based</button></li>
                </ul>
                </div> 
                <div className="dropdown">
                <button className="btn btn-secondary bg-secondary-color border-0 dropdown-toggle" 
                        type="button" id="dropdownType" data-bs-toggle="dropdown" 
                        aria-expanded="false" required>
                    Type
                </button>
                <ul className="dropdown-menu" aria-labelledby="dropdownType">
                    <li><button className="dropdown-item" type='button'
                        value='Hot' onClick={this.typeChange}>Hot</button></li>
                    <li><button className="dropdown-item" type='button'
                        value='Frozen' onClick={this.typeChange}>Frozen</button></li>
                </ul>
                </div>
            </div>
            <div className="mb-3 justify-content-center">
                <p className="form-label">Select an image:</p>
                <input className="form-control mb-3" type="file" accept="image/*" id="image"
                    name="image" onChange={this.imageChange} required/>
